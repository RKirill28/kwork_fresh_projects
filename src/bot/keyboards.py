...
def show_parser_menu(parser_state: bool) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⚙️ Настройки парсера", callback_data="settings"
                )
            ],
            [
                InlineKeyboardButton(
                    text="⏸️ Стоп" if parser_state else "▶️ Старт",
                    callback_data="parser_toggle",
                )
            ],
        ]
    )
...
def show_settings_parser_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🗂 Выбрать категории", callback_data="choice_category"
                )
            ],
            [
                InlineKeyboardButton(
                    text="⏳ Установить частоту проверки биржи",
                    callback_data="set_time_parse",
                )
            ],
...
def get_category_menu(
    cats: list[TelegramCategory] | list[TelegramSubCategory] | list[TelegramAttr],
) -> InlineKeyboardMarkup:
