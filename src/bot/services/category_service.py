...
TELEGRAM_CATEGORIES_TYPE = (
    list[TelegramCategory] | list[TelegramSubCategory] | list[TelegramAttr]
)
...
async def build_category_data(state: FSMContext, id: int) -> CategoryData:
    ...
    if curr_state == Menu.choose_main_cat.state:
        ...
        raise CategoryServiceException(
            "Не удалось собрать CategoryData из полученного id!"
        )


def set_selected_cats(
    categories: list[TelegramCategory], user_cats: set[CategoryData]
) -> list[TelegramCategory]:
    ...
    sub_ids = {
        ...
        if cat.sub_id is not None and cat.attr_id is None
    }
    attr_pairs = {
        (cat.sub_id, cat.attr_id)
        for cat in user_cats
        if cat.sub_id is not None and cat.attr_id is not None
    }
...
def change_selected(
    cats: TELEGRAM_CATEGORIES_TYPE, id: int
) -> TELEGRAM_CATEGORIES_TYPE:
    for cat in cats:
        if cat.id == id:
